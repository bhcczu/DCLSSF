...
%%%%%求投影后X向量的类内散度矩阵
% D=Wbx'*PhibX*PhibX'*Wbx;
% Xproj=Wbx'*X;
% for i = 1:c
%     index = find(label==classes(i));
%     nSample(i) = length(index);
%     cellXproj{1,i} = Xproj(:,index);
%     %cellYproj{1,i} = Yproj(:,index);
% end
% [t,z] = size(Xproj);
% classMeanXproj = zeros(t,c);
% %classMeanYproj = zeros(q,c);
% for i = 1:c
%     classMeanXproj(:,i) = mean(cellXproj{1,i},2);   % Mean of each class in X
%    % classMeanYproj(:,i) = mean(cellYproj{1,i},2);   % Mean of each class in Y
% end
% PhibWX = zeros(t,c-1);
% %PhibWY = zeros(q,c);
% for i = 1:c    
%         b=(cellXproj{1,i}-repmat(classMeanXproj(:,i),1,nSample(i)))/sqrt(nSample(i));
%         PhibWX = PhibWX+ b*b';
%        % PhibWY(:,i) = (cellYproj(1,i)-repmat(classMeanY(:,i),1,nSample(i)))/sqrt(nSample(i));  
% end
% %Swx=PhibWX*PhibWX';
% %Swy=PhibWY*PhibWY';
...
% end
% [t,z] = size(Yproj);
% classMeanYproj = zeros(t,c);
% %classMeanYproj = zeros(q,c);
% for i = 1:c
%     classMeanYproj(:,i) = mean(cellYproj{1,i},2);   % Mean of each class in X
%    % classMeanYproj(:,i) = mean(cellYproj{1,i},2);   % Mean of each class in Y
% end
% PhibWY = zeros(t,c-1);
% %PhibWY = zeros(q,c);
% for i = 1:c    
%         b=(cellYproj{1,i}-repmat(classMeanYproj(:,i),1,nSample(i)))/sqrt(nSample(i));
%         PhibWY = PhibWY+ b*b';
%        % PhibWY(:,i) = (cellYproj(1,i)-repmat(classMeanY(:,i),1,nSample(i)))/sqrt(nSample(i));  
% end
